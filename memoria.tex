...
\subsection{Registros de Alumnos y Eventos}
	Este apartado solo será visible por parte el administrador de la aplicación o del docente encargado en gestionar la semana cultural donde se impartan estas charlas y seminarios. Como vimos anteriormente, el administrador tiene la posibilidad de descargarse un archivo en formato CSV (comma-separated values, del inglés valores separados por comas) con toda la información de un Evento o de todos ellos, obteniendo un listado de los alumnos que se han apuntado a una determinada charla y que han validado su asistencia por medio de nuestra aplicación.
...
\clearpage
	...
	Un \emph{service worker} es un script
\footnote{En informática, un script es un término informal que se usa para designar a un programa relativamente simple. } que permite interceptar y controlar los requests de la red y el almacenamiento del cache del navegador. Gracias a los \textit{service workers}, los programadores web pueden crear sitios web con una experiencia offline y utilizar la aplicación sin conexión únicamente con las páginas que previamente se hayan guardado en la caché del dispositivo.
...
\item eEn el campo \textbf{Aula} se informá del aula dentro de la Universidad Rey Juan Carlos donde tendrá lugar la charla.
...
\end{figure}  	
  	...
  	Este programa tiene como objetivo la optimización y reubicación de los eventos en función de los huecos libres que quedan con respecto a la gente que se ha apuntado o quiere asistur al evento, con los huecos libres que quedan en la sala. Funciona de la siguiente manera:
